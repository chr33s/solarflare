...
/**
 * Determine module kind from file path using AST-like pattern matching
 */
 ...
 * This provides a unified, AST-inspired approach to path analysis
 */
export function parsePath(filePath: string): ParsedPath {
  ...
  const specificity = staticSegments * 2 + dynamicSegments + (pattern === '/' ? 0 : segments.length)
